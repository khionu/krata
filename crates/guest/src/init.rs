...
use crate::{
  ...
  supervisor::{ChildSpec, Retry, Strategy, Supervisor},
};
...
impl GuestInit {
  ...
  async fn early_init(&mut self) -> Result<()> {
    ...
    self.create_dir("/dev/pts", Some(0o0666)).await?;
...
std::env::set_current_dir(NEW_ROOT_PATH)?;
  ...
  async fn run(
    ...
    // TODO: impl From<oci_spec::image::Config> for ChildSpec
    ...
    self.supervisor.spawn_process(child_spec).await;

    let mut guest_bg = GuestBackground::new(idm, cgroup, Pid::from_raw(0), self.supervisor.clone()).await?;
    let fatal_fut = self.supervisor.run_to_end();

    select! {
      res = guest_bg.run() => res,
      _ = fatal_fut => Ok(()),
    }
  }
